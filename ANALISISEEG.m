...
fs= 256;
fc=[5 40];
data = importdata("EEG_#3.txt");
D1=data(1:439040,2:8);
[m,n] = size(D1);
f = (0:m-1)*((fs)/m);
t= data(1:439040,1);
...
nexttile
%plot (data.Tiempo,data.E1);
plot(f,fftxreal(:,5)); axis([0 150 0 max(fftxreal(:,4))]),grid on;
title('T4 con FFT')
xlabel('Frecuencia (Hz)')
ylabel('Magnitud')
grid on
...
nexttile
%plot (data.Tiempo,data.E1);
plot(f,fftxreal(:,6)); axis([0 150 0 max(fftxreal(:,5))]),grid on;
title('O1 con FFT')
xlabel('Frecuencia (Hz)')
ylabel('Magnitud')
